...
@onready var boundary_y : int = boundary_x/16*9
...
func _ready() -> void:
	...
	urban = generate_noise(0.005, 3, "simplex", 1, 0.4)
	#urban = generate_noise(0.01, 1, "simplex", 1, 0.2)
	destruction = generate_noise(0.003, 3, "cellular", 1, 0.2)
	_set_tile()
	...
	for x in range(-width_half, width_half):
		for y in range(-height_half, height_half):
			...
			if _urban > 0:
				...
				elif alt >= -0.25 and alt < -0.2 : place_tile_biome(pos, "eucalyptus")
